...
const types = require("babel-types");//处理类型 比如：判断某个节点是否是某种类型
const parser = require("@babel/parser");//把源代码转成抽象语法树 它是一个树形结构，来表示代码的结构
...
function buildModule(absolutePath){
  ...
  traverse(ast, {
    CallExpression({ node }) {// 捕获抽象语法树里这样的一个节点
      if (node.callee.name === 'require') {
        ...
        node.arguments = [types.stringLiteral(depModuleId)];// 将./title.js变成./src/title.js
