...
module.exports = function (webpackEnv) {
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    const loaders = [
      isEnvDevelopment && require.resolve("style-loader"),// 开发环境下，把css变成<style></style>标签包裹的形式插入到页面里，可以热更新
      isEnvProduction && {
        loader: MiniCssExtractPlugin.loader,// 压缩css的；生产环境下，对css文件进行压缩，并不会变成<style></style>标签包裹的形式插入到页面里，单独提取，可以缓存
      },
      {
      ...
      {
        loader: require.resolve("postcss-loader"),// 预处理器
      },
    ].filter(Boolean);
    if (preProcessor) {// 是否有预处理器
      loaders.push(
        {
    ...
    // 可以看到，处理css的时候，会经过这6个loader的处理，loader的执行顺序是，loaders数组里的loader从后往前开始执行，也就是逆序依次执行。
    return loaders;
  };
  return {
